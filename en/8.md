...
We can read the type of [putStrLn] like this: [putStrLn]
...
However, [foo] would just have a value of [()], so doing
...
Using [return] doesn't cause the I/O *do* block to end in
...
[putStr] is much like [putStrLn] in that it
...
So [sequence \[getLine, getLine, getLine\]] makes an I/O action
...
We could have actually done that without [forM], only with
...
We run the [getContents] I/O action and name the string it
...
What's the deal with that [Word8] type? Well, it's like
...
As you can see [empty] makes an empty bytestring. See
...
A very simple program. We perform the [getArgs] I/O action and
...
There are several predicates that act on [IOError] and if a
...
[System.IO.Error] also exports functions that enable us to ask
...
[ioe] and you can see a [full list of
...
error. We can't print the [fileName] that we got from
[getArgs], because only the [IOError] is passed to the
...
the [ioeGetFileName] function, which has a type of
[ioeGetFileName :: IOError -&gt; Maybe FilePath]. It takes an
[IOError] as a parameter and maybe returns a [FilePath]
(which is just a type synonym for [String], remember, so it's
kind of the same thing). Basically, what it does is it extracts the file
path from the [IOError], if it can. Let's modify our program to
...
In the guard where [isDoesNotExistError] is [True], we
used a *case* expression to call [ioeGetFileName] with
[e] and then pattern match against the [Maybe] value
...
[catch] or you can cover several of them with [catch]
...
Here, [toTry] uses [handler1] as the handler and
[thenTryThis] uses [handler2]. [launchRockets]
isn't a parameter to [catch], so whichever exceptions it might
throw will likely crash our program, unless [launchRockets] uses
[catch] internally to handle its own exceptions. Of course
[toTry], [thenTryThis] and [launchRockets] are
...
like [IO (Either a b)], meaning that they're normal I/O actions
but the result that they yield when performed is of type [Either a
